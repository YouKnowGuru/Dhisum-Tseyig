...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-10 stagger-in">
      {/* Header */}
      <div className="flex flex-col md:flex-row items-center justify-between bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm shadow-bhutan-maroon/5 relative overflow-hidden gap-6">
        <div className="absolute top-0 right-0 w-64 h-64 bg-bhutan-gold/5 blur-3xl -mr-32 -mt-32" />
        <div className="relative z-10 text-center md:text-left">
          <h1 className="text-3xl md:text-4xl font-black tracking-tight text-slate-900 dark:text-white leading-none mb-3">Admin Dashboard</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          ...
          <Card key={item.label} className="hover-lift border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden group rounded-3xl">
            <CardContent className="p-6 md:p-8">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{item.label}</p>
                  <p className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white group-hover:text-bhutan-maroon transition-colors">{item.value || 0}</p>
                </div>
          ...
          {data?.recentLicenses && data.recentLicenses.length > 0 ? (
            <div className="divide-y divide-slate-50">
              {data.recentLicenses.map((license) => (
                <div
                  key={license._id}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 md:p-8 hover:bg-slate-50/50 dark:bg-slate-800/50 transition-colors group gap-6"
                >
                  <div className="flex items-center gap-4 md:gap-6">
                    ...
                    <div className="overflow-hidden">
                      <p className="font-black text-base md:text-lg text-slate-800 dark:text-white tracking-tight truncate max-w-[200px] sm:max-w-none">{license.licenseKey}</p>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className="text-xs md:text-sm font-bold text-slate-500 dark:text-slate-400">{license.customerName}</span>
            ...
            <div className="text-center py-20">
              <div className="h-16 w-16 bg-slate-50 dark:bg-slate-900/50 rounded-2xl flex items-center justify-center mx-auto mb-6">
