...
const secureSettings = {
  ...
  createUser: async (userData: any) => {
    ...
    // Normalize the response — createSecureIpcHandler wraps failures as
    // { ok: false, status, data: { success: false, error: '...' } }
    // but the UI expects a flat { success, message } shape.
    const raw = await ipcRenderer.invoke('settings:createUser', sanitized);
    if (raw && raw.ok === false && raw.data) {
      return { success: false, message: raw.data.error || 'Failed to create user' };
    }
    return raw;
  },
...
const securePrint = {
  ...
  /**
   * Print report (legacy DOM-dump path — kept for backward-compat)
   */
  ...
  /**
   * Print a typed report from raw structured data — generates a proper PDF template
   * bypassing the DOM entirely, so styling is always correct.
   */
  reportData: async (reportType: string, title: string, data: any) => {
    const sanitizedType = sanitizeString(reportType, 50) || '';
    const sanitizedTitle = sanitizeString(title, 200) || '';
    return ipcRenderer.invoke('print:reportData', sanitizedType, sanitizedTitle, data);
  },
...
const secureLicense = {
  ...
  onUpdateEvent: (callback: (event: string, data?: any) => void) => {
    const channels = ['update:available', 'update:downloaded', 'update:checking', 'update:progress', 'update:error'];
    const handlers: Record<string, (_event: any, data: any) => void> = {};
    for (const channel of channels) {
      handlers[channel] = (_event: any, data: any) => callback(channel, data);
...
const secureReports = {
  ...
  /**
   * Get customer insights (top customers, monthly trend, payment-mode breakdown, lifetime value).
   * Pass an optional customerId to scope the report to a single customer.
   */
  customerInsights: async (startDate: string, endDate: string, customerId?: number) => {
    return ipcRenderer.invoke('reports:customerInsights', { startDate, endDate, customerId });
  },
};
