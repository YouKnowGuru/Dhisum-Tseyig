...
function App() {
  ...
  // List of administrative-only pages restricted from staff users
  ...
  // Auto-redirect staff users to POS/Sales page on login and guard restricted routes
  useEffect(() => {
    if (currentUser && currentUser.role !== 'admin') {
      if (currentPage === 'dashboard' || ADMIN_ONLY_PAGES.includes(currentPage)) {
        setCurrentPage('pos');
      }
    }
   ...
   */
  const renderPage = () => {
    // Security check: Guard rendering of administrative-only pages
    if (currentUser?.role !== 'admin' && (currentPage === 'dashboard' || ADMIN_ONLY_PAGES.includes(currentPage))) {
      return <POSPage />;
    }
