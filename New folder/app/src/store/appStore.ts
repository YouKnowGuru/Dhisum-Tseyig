...
interface AppState {
  ...
  // Notifications
  notification: { message: string; type: 'success' | 'error' | 'info' | 'pos' } | null;
  showNotification: (message: string, type: 'success' | 'error' | 'info' | 'pos') => void;
  ...
  setUpdateInfo: (
    info:
      | AppState['updateInfo']
      | ((prev: AppState['updateInfo']) => AppState['updateInfo'])
  ) => void;
}
    ...
    set((state) => ({
      updateInfo:
        typeof info === 'function'
          ? (info as (prev: AppState['updateInfo']) => AppState['updateInfo'])(
              state.updateInfo
            )
          : info,
    })),
