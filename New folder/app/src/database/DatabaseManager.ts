 ...
 * SECURITY: Full-database encryption — the DB file is encrypted on disk
 * with AES-256-CBC + HMAC when the app is not running.
 */
...
export class DatabaseManager {
  ...
  constructor(dbPath: string) {
    ...
    // SECURITY: Decrypt the encrypted on-disk file to the working file.
    // If the working file already exists (crash recovery or legacy migration),
    // use it directly. Otherwise decrypt .secure → .db.
    this.decryptToWorkingFile();
   ...
   */
  reopen(): void {
    // SECURITY: In case the working file was encrypted and deleted, decrypt it back.
    this.decryptToWorkingFile();
  ...
  /**
   * SECURITY: Close the database and encrypt the working file to .secure,
   * then securely delete the plaintext working file. Called on app shutdown.
   *
   * If the DB is already closed (e.g. after restore), just encrypts the file.
   * If encryption fails, leaves the plaintext file in place (better than data loss).
   */
  closeAndEncrypt(): void {
    try {
      ...
      // DB might already be closed — that's fine, continue to encrypt the file
    }
    ...
    try {
      ...
      console.error('[DatabaseManager] Failed to encrypt database on shutdown:', error);
      // Leave plaintext file in place — better than losing data
    }
  }

  /**
   * SECURITY: Decrypt the .secure file to the working .db file.
   * Called from the constructor and reopen().
   *
   * - If .db exists and is valid SQLite → use it directly (crash recovery or legacy)
   * - If .db doesn't exist but .secure does → decrypt .secure → .db
   * - If neither exists → do nothing (new database will be created)
   */
  private decryptToWorkingFile(): void {
    // Check if working file exists and is valid SQLite
    if (fs.existsSync(this.dbPath)) {
      try {
        ...
        if (header.subarray(0, 15).toString('utf8') === 'SQLite format 3') {
          // Valid SQLite working file — use directly (crash recovery or legacy migration)
          return;
        }
      } catch {
        // Can't read header — try secure file
      }
    }

    // Working file doesn't exist or is invalid — try decrypting .secure
    if (fs.existsSync(this.securePath)) {
      try {
        const encrypted = fs.readFileSync(this.securePath);
        const decrypted = decryptBuffer(encrypted);
        fs.writeFileSync(this.dbPath, decrypted, { mode: 0o600 });
        console.log('[DatabaseManager] Decrypted database from .secure file.');
      } catch (error) {
        console.error('[DatabaseManager] Failed to decrypt .secure file:', error);
        // Leave .secure in place — don't delete it, it might be recoverable
        // The constructor will create a new empty database
      }
    }
  }

  /**
   * SECURITY: Overwrite a file with zeros before deleting it to prevent
   * recovery of plaintext data by disk forensics tools.
   */
  private secureDeleteFile(filePath: string): void {
    try {
      const stats = fs.statSync(filePath);
      const fd = fs.openSync(filePath, 'w');
      fs.writeSync(fd, Buffer.alloc(stats.size, 0));
      fs.closeSync(fd);
    } catch {
      // If overwrite fails, just try to delete
    } finally {
  ...
  importDatabaseFromJSON(data: any): boolean {
    ...
    try {
      ...
      try { this.db.pragma('foreign_keys = ON'); } catch (_e) { /* intentionally empty */ }
    }
  }
  ...
  async importDatabaseFromJSONStream(
    ...
    const tableNames = await this.extractTableNames(inputPath, parser);
  ...
  private initializeSchema(): void {
    ...
    `);
    // Migration: ensure gst_rate column exists on legacy databases
    try {
      this.db.exec(`ALTER TABLE transaction_lines ADD COLUMN gst_rate REAL DEFAULT 0;`);
    } catch { /* Column already exists, ignore */ }
    ...
    // Migration: add is_void column to refunds for soft-delete (void cascade)
    try {
      this.db.exec(`ALTER TABLE refunds ADD COLUMN is_void INTEGER DEFAULT 0;`);
    } catch { /* Column already exists */ }
  ...
  private seedDefaultCategories(): void {
    ...
    for (const cat of categories) {
      try { insert.run(cat); } catch (_e) { /* intentionally empty */ }
    }
  }
  ...
  private seedDefaultUnits(): void {
    ...
    for (const unit of units) {
      try { insert.run(unit); } catch (_e) { /* intentionally empty */ }
    }
  }
  ...
  private runMigrations(): void {
    ...
    // SECURITY: Tamper-evident audit log hash chaining
    const auditSchema = this.db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='audit_logs'").get() as any;
    if (auditSchema && auditSchema.sql && !auditSchema.sql.includes('prev_hash')) {
      console.log('[DatabaseManager] Migration: Add prev_hash, row_hash columns to audit_logs...');
      try {
        this.db.exec('ALTER TABLE audit_logs ADD COLUMN prev_hash TEXT');
        this.db.exec('ALTER TABLE audit_logs ADD COLUMN row_hash TEXT');
        console.log('[DatabaseManager] Audit log hash chain columns added.');
      } catch (error) {
        console.error('[DatabaseManager] Failed to add audit hash columns:', error);
      }
    }

    // SECURITY: Integrity hashes table for critical data (settings, users, period_locks)
    this.db.exec(`
    ...
    // Migration: ensure all local staff accounts are marked as verified since they are created by the admin
    try {
      this.db.prepare("UPDATE users SET is_verified = 1 WHERE role = 'staff' AND is_verified = 0").run();
    } catch (error) {
      console.warn('Failed to verify staff accounts:', error);
    }
  }
}
