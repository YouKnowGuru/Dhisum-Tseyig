...
export class QuotationService {
  ...
  create(data: CreateQuotationData): ApiResponse<{ id: number }> {
    try {
      ...
      this.audit.logAction({
        ...
        entityId: quoteId,
        newValues: { quoteNo, customerId: data.customerId, totalAmount: subtotal + gstAmount }
      });
  ...
  updateStatus(id: number, status: string): ApiResponse {
    try {
      ...
      this.audit.logAction({
        ...
        entityType: 'quotations',
        entityId: id,
        newValues: { status, previousStatus: current.status }
      });
  ...
  delete(id: number): ApiResponse {
    try {
      const quote = this.db.prepare('SELECT quote_no FROM quotations WHERE id = ?').get(id) as any;
      ...
      this.audit.logAction({
        ...
        entityType: 'quotations',
        entityId: id,
        newValues: { quoteNo: quote?.quote_no }
      });
