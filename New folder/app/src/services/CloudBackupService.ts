...
export class CloudBackupService {
  ...
  /**
   * SECURITY FIX: Encrypt JSON content before writing to disk so settings
   * and logs cannot be read or edited as plaintext. Uses AES-256-GCM with
   * a key derived from the per-install secret.
   * Format: `<iv-hex>:<ciphertext-hex>:<authTag-hex>`
   */
  private encryptJson(jsonStr: string): string {
    const secret = getInstallSecret();
    if (!secret) return jsonStr; // no secret — fall back to plaintext
  ...
  /**
   * SECURITY FIX: Decrypt content written by encryptJson(). Returns null
   * if the content is not in encrypted format (legacy plaintext) or
   * decryption fails, so the caller can fall back to plaintext JSON.
   */
  private decryptJson(raw: string): string | null {
  ...
  saveSettings(newSettings: Partial<CloudBackupSettings>): void {
    ...
    try {
      const encrypted = this.encryptJson(JSON.stringify(this.settings, null, 2));
      fs.writeFileSync(this.settingsPath, encrypted, 'utf-8');
  ...
  private loadSettings(): CloudBackupSettings {
    try {
      if (fs.existsSync(this.settingsPath)) {
        ...
        const decrypted = this.decryptJson(raw);
        const jsonStr = decrypted ?? raw; // fall back to plaintext (legacy)
  ...
  private saveLogs(): void {
    try {
      const encrypted = this.encryptJson(JSON.stringify(this.logs, null, 2));
      fs.writeFileSync(this.logsPath, encrypted, 'utf-8');
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(emergencyPath)) {
          try { fs.unlinkSync(emergencyPath); } catch (_e) { // intentionally empty
 }
        }
      }
