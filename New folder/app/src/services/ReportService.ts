...
export class ReportService {
  ...
  getDashboardData(): ApiResponse<DashboardData> {
    try {
      ...
      // Today's sales (Net sales: Sales minus Refunds)
      // Refunds are stored as type='adjustment'; identified reliably via the refunds table.
      ...
      const todaySales = this.db.prepare(`
        SELECT COALESCE(SUM(
          CASE
            ...
            WHEN type = 'adjustment' AND id IN (SELECT transaction_id FROM refunds) THEN -net_amount
            ELSE 0
          END
        ), 0) as total
        FROM transactions
        WHERE (type = 'sale' OR (type = 'adjustment' AND id IN (SELECT transaction_id FROM refunds)))
          AND date = ? AND is_void = 0
      `).get(today);
      ...
      // Monthly sales (Net) - Sales minus Refunds
      // Refunds are stored as type='adjustment'; identified reliably via the refunds table.
      // Receipts are payments against existing credit sales — counting them would double-count
      ...
      // Sales trend (last 30 days) - Sales minus Refunds
      // Refunds are stored as type='adjustment'; identified reliably via the refunds table.
      // Receipts are payments against existing credit sales — counting them would double-count
      ...
      // Payment mode breakdown (current month) - Sales minus Refunds
      // Refunds are stored as type='adjustment'; identified reliably via the refunds table.
      // Receipts are payments against existing credit sales — counting them would double-count
  ...
  getSalesReport(startDate: string, endDate: string): ApiResponse<any> {
    try {
      ...
      // Refunds use type='adjustment'; identified reliably via the refunds table (transaction_id FK).
        ...
        SELECT
          ...
          COALESCE(SUM(
            CASE 
              ...
              WHEN type = 'adjustment' AND id IN (SELECT transaction_id FROM refunds) THEN -(net_amount - gst_amount)
              ELSE 0 
            END
          ), 0) as total_revenue,
          COALESCE(SUM(
          ...
          COALESCE(SUM(
            CASE 
              ...
              WHEN type = 'adjustment' AND id IN (SELECT transaction_id FROM refunds) THEN -gst_amount
              ELSE 0 
            END
          ), 0) as total_gst,
          COALESCE(AVG(
        ...
        SELECT
          ...
          SUM(
            CASE 
              ...
              WHEN t.type = 'adjustment' AND t.id IN (SELECT transaction_id FROM refunds) THEN -t.net_amount
              ELSE 0 
            END
          ) as total_purchases
        FROM transactions t
  ...
  // Shows total PURCHASES per customer (all payment modes: cash/bank/card/credit/etc.),
  // NOT the ledger balance. Answers "who buys the most" rather than "who owes money".
  // Sales = type 'sale'; Refunds = type 'adjustment' linked via refunds table (netted out).
  // ============================================

  getCustomerInsights(startDate: string, endDate: string, customerId?: number): ApiResponse<any> {
    try {
      // Sales = type 'sale'; Refunds = type 'adjustment' linked via the refunds table (netted out).
      // Receipts are excluded — they settle existing credit sales and would double-count as
      // "purchases". This answers "who buys the most" across ALL payment modes
      // (cash / bank / card / credit / mBOB / BNB / T-Pay / etc.), NOT the ledger balance.
      // When customerId is provided, all queries are scoped to that single customer.
      const isSaleOrRefund = `(t.type = 'sale' OR (t.type = 'adjustment' AND t.id IN (SELECT transaction_id FROM refunds)))`;
      const custFilter = customerId ? ` AND t.contact_id = ?` : '';
      const custFilterPlain = customerId ? ` AND contact_id = ?` : '';
      const summaryParams = customerId ? [startDate, endDate, customerId] : [startDate, endDate];
      const queryParams = customerId ? [startDate, endDate, customerId] : [startDate, endDate];

      // --- Summary (scoped to one customer when filtered) ---
      const summary = this.db.prepare(`
        SELECT
          COUNT(DISTINCT contact_id) as active_customers,
          COUNT(*) as total_transactions,
          COALESCE(SUM(
            CASE
              WHEN type = 'sale' THEN net_amount
        ...
        WHERE (type = 'sale' OR (type = 'adjustment' AND id IN (SELECT transaction_id FROM refunds)))
          AND date BETWEEN ? AND ? AND is_void = 0
          AND contact_id IS NOT NULL${custFilterPlain}
      `).get(...summaryParams);

      // --- Top customers for the period (ranked by net purchases) ---
      // When filtered to a single customer, this returns just that one row.
      const topCustomers = this.db.prepare(`
        SELECT
          c.id,
          c.name,
          ...
          COALESCE(SUM(
            CASE
              ...
              WHEN t.type = 'adjustment' AND t.id IN (SELECT transaction_id FROM refunds) THEN -(t.net_amount - t.gst_amount)
              ELSE 0
            END
          ), 0) as total_revenue,
        ...
        FROM transactions t
        JOIN contacts c ON t.contact_id = c.id
        WHERE ${isSaleOrRefund}
          AND t.date BETWEEN ? AND ? AND t.is_void = 0${custFilter}
        GROUP BY c.id
        ORDER BY total_purchases DESC
        ${customerId ? '' : 'LIMIT 20'}
      `).all(...queryParams);

      // --- Payment-mode breakdown per customer (for the period) ---
      const customerModes = this.db.prepare(`
        SELECT
          ...
          CASE t.payment_mode
            ...
            WHEN 'BDBL' THEN 'BDBL'
            WHEN 'DKBank' THEN 'DKid'
            ELSE COALESCE(t.payment_mode, 'Unknown')
          END as mode,
          COALESCE(SUM(
            CASE
              WHEN t.type = 'sale' THEN t.net_amount
        ...
        WHERE ${isSaleOrRefund}
          AND t.date BETWEEN ? AND ? AND t.is_void = 0
          AND t.contact_id IS NOT NULL${custFilter}
        GROUP BY t.contact_id, mode
      `).all(...queryParams);

      // Group modes by customer id for easy lookup
      const modesByCustomer: Record<number, any[]> = {};
      for (const m of customerModes as any[]) {
        if (!modesByCustomer[m.customer_id]) modesByCustomer[m.customer_id] = [];
        modesByCustomer[m.customer_id].push({ mode: m.mode, amount: Number(m.amount || 0), count: m.txn_count });
      }

      // --- Lifetime value (all-time, all dates) for the top customers in the period ---
      const topCustomerIds = (topCustomers as any[]).map((c: any) => c.id);
      const lifetimeByCustomer: Record<number, number> = {};
      if (topCustomerIds.length > 0) {
        const placeholders = topCustomerIds.map(() => '?').join(',');
        const lifetimeRows = this.db.prepare(`
          SELECT
            t.contact_id as customer_id,
            COALESCE(SUM(
              CASE
        ...
        for (const r of lifetimeRows as any[]) {
          lifetimeByCustomer[r.customer_id] = Number(r.lifetime || 0);
        }
      }

      // --- Monthly trend (per month in range, scoped when filtered) ---
      ...
      // --- Payment-mode split (for the period, scoped when filtered) ---
      const paymentModeSplit = this.db.prepare(`
        SELECT
          CASE payment_mode
            WHEN 'cash' THEN 'Cash'
            WHEN 'bank' THEN 'Bank'
            ...
            WHEN 'BDBL' THEN 'BDBL'
            WHEN 'DKBank' THEN 'DKid'
            ELSE COALESCE(payment_mode, 'Unknown')
          END as mode,
          COALESCE(SUM(
            CASE
              WHEN type = 'sale' THEN net_amount
      ...
      const activeCustomers = Number((summary as any)?.active_customers || 0);
      const totalPurchases = Number((summary as any)?.total_purchases || 0);
      const totalRevenue = Number((summary as any)?.total_revenue || 0);
      const totalTransactions = Number((summary as any)?.total_transactions || 0);

      // --- Per-customer transaction list (only when filtered to one customer) ---
      let transactions: any[] = [];
      let customer: any = null;
      if (customerId) {
        const contactRow = this.db.prepare(`SELECT id, name, phone, email, address, gst_number FROM contacts WHERE id = ?`).get(customerId) as any;
        customer = contactRow ? {
          id: contactRow.id,
          ...
          gstNumber: contactRow.gst_number,
        } : null;
        ...
        `).all(startDate, endDate, customerId) as any[]).map((t: any) => ({
          id: t.id,
          ...
          signedTotal: Number(t.signed_total || 0),
          isRefund: t.type === 'adjustment',
        }));
      }
      ...
      return {
        ...
        data: {
          ...
          summary: {
            ...
            totalPurchases,
            totalRevenue,
            averagePerCustomer: activeCustomers > 0 ? totalPurchases / activeCustomers : 0,
            averageSale: totalTransactions > 0 ? totalPurchases / totalTransactions : 0,
          },
          topCustomers: (topCustomers as any[]).map((c: any) => ({
            ...
            lifetimeValue: lifetimeByCustomer[c.id] || 0,
            paymentModes: modesByCustomer[c.id] || [],
          })),
          transactions,
          monthlyTrend: (monthlyTrend as any[]).map((m: any) => ({
            ...
            totalPurchases: Number(m.total_purchases || 0),
          })),
          paymentModeSplit: (paymentModeSplit as any[]).map((p: any) => ({
            mode: p.mode,
            amount: Number(p.amount || 0),
            count: Number(p.txn_count || 0),
          })),
        }
      };
    } catch (error: any) {
      console.error('Get customer insights error:', error);
      return { success: false, message: 'Failed to generate customer insights: ' + error.message };
    }
  }
