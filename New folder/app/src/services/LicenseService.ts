...
const TRIAL_DAYS = 7;
// BUG FIX H-7: reduce offline grace period from 30 → 7 days. Even within
// this window, an explicit server-side denial (valid=false) immediately
// blocks write operations, see `lastDeniedAt` checks below.
...
export class LicenseService {
    ...
    getStatus(): LicenseStatus {
        ...
        if (license) {
            ...
            // BUG FIX H-7: even if last verified within the grace window,
            // if the server *explicitly denied* us recently, refuse use
            // until we can reach the server again.
            if (license.lastDeniedAt) {
                const deniedAt = new Date(license.lastDeniedAt);
                const daysSinceDenial = Math.floor((now.getTime() - deniedAt.getTime()) / (1000 * 60 * 60 * 24));
                if (daysSinceDenial <= OFFLINE_GRACE_DAYS) {
                    return {
                        ...baseStatus,
                        type: 'license_expired',
                        plan: license.plan,
                        message: 'License was rejected by the server. Reconnect to verify.',
                    };
                }
            }
    ...
    async verifyWithServer(): Promise<boolean> {
        ...
        try {
            ...
            if (data.valid) {
                ...
                // BUG FIX H-7: a successful verification clears any prior
                // server-side denial.
                license.lastDeniedAt = undefined;
            ...
            // BUG FIX H-7: server explicitly denied us — record the rejection
            // so the client can refuse write operations even while still
            // within the 7-day grace window.
            license.lastDeniedAt = new Date().toISOString();
            writeLicense(license);
