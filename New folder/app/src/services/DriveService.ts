...
export class DriveService {
   ...
   *
   * BUG FIX C-3: accepts an opaque `state` parameter that MUST be verified
   * on the redirect callback to prevent OAuth CSRF / authorization-code
   * injection.
   */
  getAuthUrl(state?: string): string {
    ...
    return this.oauth2Client.generateAuthUrl({
      ...
      // google-auth-library appends `state` to the URL when provided.
      ...(state ? { state } : {}),
      // CSRF protection: include_state flag is a hint but is NOT sufficient on
      // its own — caller must verify `state` matches on the redirect.
