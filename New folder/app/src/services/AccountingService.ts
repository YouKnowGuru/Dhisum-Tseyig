...
export class AccountingService {
    ...
    try {
      ...
      if (user) {
        ...
        if (isVerified) {
          ...
          // BUG FIX M-2: use async bcrypt to avoid blocking the Electron event loop.
          ...
          if (!validPassword) {
            ...
            console.log('[Auth] Attempting cloud credential sync...');
            ...
            if (cloudResult.success && cloudResult.user) {
              ...
              // BUG FIX M-2: use async bcrypt; cost-factor 12 enforces ~250ms CPU work off the event loop.
              ...
              this.audit.logAction({
                ...
                entityType: 'users',
                entityId: user.id,
                newValues: { username: credentials.username, reason: cloudResult.message || 'Invalid email or password' }
              });
          ...
          console.log('[Auth] Unverified user, checking cloud status...');
          const cloudResult = await this.performCloudLogin(user.email || credentials.username, credentials.password);
          ...
          if (cloudResult.success && cloudResult.user && cloudResult.user.isVerified) {
            ...
            // BUG FIX M-2: async bcrypt keeps the event loop free.
            const passwordHash = await bcrypt.hash(credentials.password, 12);
            this.db.prepare('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
              .run(passwordHash, user.id);
          } else {
            this.audit.logAction({
              ...
              entityType: 'users',
              entityId: user.id,
              newValues: { username: credentials.username, reason: 'Email not verified' }
            });
            return {
              success: false,
        ...
        if (cloudResult.success && cloudResult.user) {
          ...
          // BUG FIX M-2: async bcrypt so the Electron main process is not blocked.
          const passwordHash = await bcrypt.hash(credentials.password, 12);
          ...
          this.audit.logAction({
            action: 'LOGIN_FAILED',
            entityType: 'users',
            newValues: { username: credentials.username, reason: cloudResult.message || 'Invalid username, email or password' }
          });
  ...
  /**
   * BUG FIX M-2: made async so bcrypt hashing does not block the Electron
   * main process event loop.
   */
  async syncLocalPassword(email: string, password: string): Promise<ApiResponse> {
    try {
  ...
  /**
   * BUG FIX M-2: made async to avoid blocking main thread on bcrypt hash.
   */
  async changePassword(data: { userId: number; newPassword: string; oldPassword?: string }): Promise<ApiResponse> {
    try {
      if (!this.currentUser) {
        return { success: false, message: 'Not authenticated' };
      }

      // Zod validator already enforces min 8 chars, max 128, complexity rules
      if (!data.newPassword) {
      ...
      if (this.currentUser.id === data.userId || this.currentUser.role !== 'admin') {
        ...
        const validPassword = await bcrypt.compare(data.oldPassword, targetUser.password_hash);
        if (!validPassword) {
  ...
  getContactLedger(contactId: number): ApiResponse<LedgerData> {
    try {
      ...
      `).all(contact.accountId, contactId, contact.accountId, contactId, contactId);
  ...
  createSale(data: SaleData): ApiResponse<any> {
    try {
      ...
      // BUG-A04 FIX: Double-entry balance for discounted sales requires:
      //   Debits:  Cash/AR (netAmount) + Discount Allowed (discountAmount) + COGS
      //   Credits: Sales Revenue (full subtotal) + GST Output + COGS
      //
      // Using revenueAfterDiscount for the revenue credit caused the debit side to be
      // exactly discountAmount MORE than the credit side, which failed validateBalance().

      const lines: EngineTransactionLine[] = [];

      // Debit Cash/Customer — the net amount actually collected
      lines.push({
      ...
      // Credit Sales Revenue — FULL subtotal (discount debit absorbs the difference)
      lines.push({
        accountId: mapping.creditAccount,
        description: 'Sales Revenue',
        debitAmount: 0,
        creditAmount: subtotal   // Full pre-discount amount so debits == credits
      });

      // Credit GST Output — emit one line per distinct GST rate so mixed-rate
      // sales (e.g. 5% standard + 0% exempt with a single domestic rate) are
      // recorded at the correct rate on each transaction_lines row. The engine
      // groups gst_entries by rate downstream, but the journal itself needs
      // a single line per rate.
      if (totalGst > 0) {
        const gstAccountId = this.automation.getGstAccount();
        const gstByRate = new Map<number, number>();
        for (const eng of engineItems) {
          const amt = Number(eng.gstAmount) || 0;
          if (amt <= 0) continue;
          const rate = Number(eng.gstRate) || 0;
          gstByRate.set(rate, (gstByRate.get(rate) || 0) + amt);
        }
        if (gstByRate.size === 0) {
          // Fallback: lump into a single line using the sale's effective rate.
          const fallbackRate = taxType === 'domestic' ? domesticGstRate : (data.items[0]?.gstRate ?? 5.0);
          gstByRate.set(fallbackRate, totalGst);
        }
        for (const [rate, amt] of gstByRate.entries()) {
          const rounded = Number(amt.toFixed(2));
          if (rounded <= 0) continue;
          lines.push({
            accountId: gstAccountId,
            description: `GST Output (${rate}%)`,
            debitAmount: 0,
  ...
  transferMoney(data: TransferData): ApiResponse {
    try {
      ...
      // The source-account balance must be computed against its *normal side*:
      //   debit-normal accounts (asset/expense): balance = sum(debit - credit)
      //   credit-normal (liability/equity/income): balance = sum(credit - debit)
      // We also exclude voided transactions to match the engine's view of balance.
      const fromAccount = this.db.prepare(`
        SELECT a.id, a.name, a.code, a.type,
          COALESCE(SUM(
            CASE
              WHEN a.type IN ('asset', 'expense') THEN (tl.debit_amount - tl.credit_amount)
  ...
  voidTransaction(id: number, reason: string): ApiResponse {
    try {
      ...
      // Reject if no authenticated user — previously fell back to userId 1,
      // producing an inaccurate audit trail.
      if (!this.currentUser?.id) {
        return { success: false, message: 'Cannot void transaction: no authenticated user.' };
      }

      // Execute void atomically using AutomationService
      this.automation.handleVoid(id, this.currentUser.id, reason);
  ...
  createOpeningBalanceEntry(contactAccountId: number, balance: number, type: 'customer' | 'supplier'): void {
    const equityAccount = this.db.prepare('SELECT id FROM accounts WHERE code = ?').get('3100') as any; // Retained Earnings / Opening Balance Equity

    // BUG-A03 FIX: Guard against missing equity account to prevent TypeError: Cannot read properties of null
    if (!equityAccount) {
      throw new Error('Opening Balance Equity account (code 3100) not found. Please set up your Chart of Accounts before adding contacts with opening balances.');
    }

    this.dbManager.safeTransaction(() => {
      // Period-lock guard: opening balances for periods that have already been
      // closed must be rejected up-front to match the engine's policy.
      const now = new Date();
      const yyyyMmDd = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
      const txDate = new Date(yyyyMmDd);
      const lockedDue = this.db.prepare(
        'SELECT 1 FROM period_locks WHERE year = ? AND month = ? AND is_locked = 1'
      ).get(txDate.getFullYear(), txDate.getMonth() + 1);
      if (lockedDue) {
        throw new Error('Period Lock: Cannot post opening balance in a locked period.');
      }

      // Validate balancing before insert. Both cases (customer / supplier) must
      // produce a journal whose debits equal credits.
      const expectedDr = type === 'customer' ? balance : 0;
      const expectedCr = type === 'customer' ? 0 : balance;
      const offsetDr = type === 'customer' ? 0 : balance;
      const offsetCr = type === 'customer' ? balance : 0;
      if (Math.abs(expectedDr + offsetDr - (expectedCr + offsetCr)) >= 0.005) {
        throw new Error('Opening Balance journal would be unbalanced — aborting.');
      }
      ...
      this.audit.logAction({
        ...
        entityType: 'transactions',
        entityId: id,
        newValues: { contactAccountId, balance, type }
      });
