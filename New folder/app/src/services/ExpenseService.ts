...
export class ExpenseService {
  ...
  delete(id: number, userId: number = 1): ApiResponse {
    try {
      const expense = this.db.prepare('SELECT transaction_id, expense_no, category, amount FROM expenses WHERE id = ?').get(id) as any;
      ...
      this.audit.logAction({
        ...
        entityType: 'expenses',
        entityId: id,
        newValues: { expenseNo: expense?.expense_no, category: expense?.category, amount: expense?.amount }
      });
