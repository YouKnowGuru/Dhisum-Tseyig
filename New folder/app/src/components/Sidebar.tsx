...
const menuItems: MenuItem[] = [
  ...
  // New Features
  { id: 'purchase-orders', label: 'Purchase Orders', icon: FilePlus, group: 'Procurement', adminOnly: true },
  ...
  { id: 'expense-tracker', label: 'Expenses', icon: TrendingDown, group: 'Finance', adminOnly: true },
  { id: 'recurring', label: 'Recurring', icon: Clock, group: 'Finance', adminOnly: true },
  { id: 'aged-reports', label: 'Aged Reports', icon: BarChart3, group: 'Finance', adminOnly: true },
  { id: 'tiered-pricing', label: 'Pricing', icon: DollarSign, group: 'Finance', adminOnly: true },
  { id: 'employees', label: 'Employees', icon: UserCheck, group: 'HR', adminOnly: true },
  { id: 'branches', label: 'Branches', icon: Building2, group: 'HR', adminOnly: true },
  { id: 'import-export', label: 'Import/Export', icon: FileText, group: 'Tools', adminOnly: true },
  { id: 'audit-log', label: 'Audit Log', icon: Shield, group: 'Security', adminOnly: true },
  { id: 'barcode', label: 'Barcodes', icon: QrCode, group: 'Tools' },

  // Compliance
  { id: 'gst', label: 'GST', icon: Calculator, adminOnly: true },
...
export function Sidebar({ currentPage, onPageChange, userRole }: SidebarProps) {
  ...
  const allMenuItems = (userRole === 'admin' ? [...menuItems, ...adminItems] : menuItems)
        ...
        <div className="p-6 border-t border-white/10">
          <div className="text-xs text-white/40 font-medium tracking-tight">
            <p className="flex justify-between items-center">
              ...
              <span className="text-white/60">{appVersion}</span>
            </p>
          </div>
        </div>
      </div>
