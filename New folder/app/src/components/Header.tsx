...
import { Bell, User, LogOut, AlertTriangle, AlertCircle, Package, X, Volume2, VolumeX } from 'lucide-react';
...
export function Header({ user, onLogout, currentPage, onNavigate }: HeaderProps) {
  ...
  const { headerAlerts, loadHeaderAlerts, soundMuted, setSoundMuted } = useAppStore();
  // Stabilise the callback reference so the interval useEffect only fires once
  const loadRef = useRef(loadHeaderAlerts);
  useEffect(() => { loadRef.current = loadHeaderAlerts; });

  // Load on mount and poll every 60 s — empty deps prevents infinite re-registration
  useEffect(() => {
    loadRef.current();
  ...
  // Track previous alert count to trigger audio notification on new alerts
  const prevAlertsCount = useRef(headerAlerts.length);
  useEffect(() => {
    if (headerAlerts.length > prevAlertsCount.current) {
      if (!soundMuted) {
    ...
    prevAlertsCount.current = headerAlerts.length;
  }, [headerAlerts.length, soundMuted]);
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-1 hover:bg-gray-100 rounded-lg transition-colors text-slate-400 hover:text-slate-600"
