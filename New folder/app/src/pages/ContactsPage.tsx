...
export function ContactsPage({ type }: ContactsPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let result;
      if (editingContactId) {
        result = await window.electronSecureAPI.contacts.update(editingContactId, payload);
      } else {
        result = await window.electronSecureAPI.contacts.create(payload);
      }
      ...
      showNotification(error.message || `Failed to ${editingContactId ? 'update' : 'create'}`, 'error');
    }
  };
  ...
  const handleEdit = (contact: Contact) => {
    ...
    setFormData({
      ...
      addressStreet: contact.addressStructured?.street || '',
      addressGewog: contact.addressStructured?.gewog || '',
      addressDzongkhag: contact.addressStructured?.dzongkhag || '',
      creditLimit: contact.creditLimit ?? 50000,
      creditDays: contact.creditDays ?? 30,
            ...
            <div className="flex items-start justify-between mb-5">
              ...
              <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                ...
                <button
                  ...
                  className="p-1.5 text-blue-500 bg-blue-50 hover:bg-blue-500 hover:text-white rounded-lg transition-colors"
                  title="Edit"
                >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold truncate text-slate-800">
                    {editingContactId ? 'Edit' : 'New'} {type === 'customer' ? 'Customer' : 'Supplier'}
                  </h3>
                  <p className="text-sm text-slate-500 font-medium">{editingContactId ? 'Update details for this trade partner' : 'Enter details for the new trade partner'}</p>
                </div>
              </div>
              <button
                onClick={() => { setShowAddModal(false); setEditingContactId(null); resetForm(); }}
