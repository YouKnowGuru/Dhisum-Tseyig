                          ...
                          <button
                            onClick={async () => {
                              if (confirm('Delete this recurring transaction?')) {
                                const result = await window.electronSecureAPI.recurring?.delete(r.id);
                                if (result?.success) {
                                  showNotification('Recurring transaction deleted', 'success');
                                  loadRecurring();
                                } else {
                                  showNotification(result?.message || result?.error || 'Failed to delete', 'error');
                                }
                              }
                            }}
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    value={formData.type}
                    onChange={(e) => setFormData({ ...formData, type: e.target.value, accountId: 0 })}
                    className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-400 outline-none"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-xs font-black text-slate-600 uppercase tracking-widest">
                    {formData.type === 'payment' ? 'Expense Account' : 'Income Account'} *
                  </label>
                  <select
                    ...
                    required
                  >
                    <option value={0}>Select {formData.type === 'payment' ? 'expense' : 'income'} account…</option>
                    {safeArray(accounts)
                      .filter(a => a.type === (formData.type === 'payment' ? 'expense' : 'income'))
                      .map(a => (
