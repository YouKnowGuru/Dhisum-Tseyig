...
export function SettingsPage() {
      ...
      const unsubscribe = window.electronSecureAPI.update.onUpdateEvent((event: string, data: any) => {
        ...
        switch (event) {
          ...
          case 'update:progress':
            // BUG FIX: Ensure available=true and downloaded=false so the
            // progress bar section (which requires available && !downloaded)
            // actually renders. Without this, if the event arrives before
            // update:available or after a state reset, the bar never shows.
            setUpdateState((prev: any) => ({ ...prev, data: { ...prev?.data, available: true, downloaded: false, progress: data?.percent, ...data } }));
            break;
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session && currentUser && currentUser.id === passwordChangeModal.userId) {
        ...
        // @ts-expect-error - TODO: Implement changeSaaSPassword properly or define it in AuthService
  ...
  const handleCheckForUpdates = async () => {
    ...
    try {
      ...
      // BUG FIX: Merge with existing state instead of replacing it entirely.
      // autoUpdater.checkForUpdates() resolves BEFORE the download finishes,
      // so the returned state doesn't include `progress`. If we replace the
      // whole state, any progress already pushed by the update:progress event
      // is lost, making the progress bar disappear.
      setUpdateState((prev: any) => ({
        ...result,
        data: {
          ...prev?.data,
          ...result?.data,
          // Preserve progress from events if the check result doesn't have it
          progress: result?.data?.progress ?? prev?.data?.progress,
        },
      }));
  ...
  const handleInstallUpdate = async () => {
    try {
      ...
      if (!result?.success) {
        // BUG FIX: The createSecureIpcHandler wraps auth/permission errors as
        // { ok: false, status, data: { success: false, error } }. Extract the
        // real error message instead of showing a generic "Failed to install".
        const errMsg = result?.data?.error || result?.message || 'Failed to install update';
        showNotification(errMsg, 'error');
      }
    } catch (err: any) {
      showNotification(err?.message || 'Failed to install update', 'error');
    }
  };
        ...
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-blue-700 font-bold">Your plan allows up to <span className="text-blue-900">{Array.isArray(usersList) ? usersList.filter(u => u.is_active && u.role !== 'admin').length : 0} / {userLimit}</span> active staff accounts.</p>
                </div>
              </div>
              <button
            ...
            {showAddUserModal && (() => {
              ...
              const strengthLabel = score <= 1 ? 'Weak' : score === 2 ? 'Fair' : score === 3 ? 'Good' : score === 4 ? 'Strong' : score === 5 ? 'Very Strong' : '';
              const strengthColor = score <= 1 ? 'bg-red-500' : score === 2 ? 'bg-orange-400' : score === 3 ? 'bg-yellow-400' : score === 4 ? 'bg-emerald-400' : 'bg-emerald-500';
              const strengthText = score <= 1 ? 'text-red-500' : score === 2 ? 'text-orange-400' : score === 3 ? 'text-yellow-500' : 'text-emerald-500';
              const initials = newUserForm.fullName.trim().split(' ').filter(Boolean).map(w => w[0]).join('').toUpperCase().slice(0, 2);

              return (
                <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl border border-slate-100 animate-in zoom-in-95 fade-in duration-200 overflow-hidden">

                    {/* Header banner */}
                    <div className="relative bg-gradient-to-br from-bhutan-maroon via-[#8B1A2E] to-[#3D0B16] px-8 pt-8 pb-12">
                      ...
                      <div className="flex items-start justify-between relative">
                        <div className="flex items-center gap-4">
                          {/* Avatar preview */}
                          <div className="w-14 h-14 rounded-2xl bg-white/20 border-2 border-white/30 flex items-center justify-center backdrop-blur-sm transition-all duration-300">
                            {initials ? (
                          ...
                          <div>
                            <h3 className="text-xl font-black text-white tracking-tight">
                              {newUserForm.fullName.trim() || 'New Staff Member'}
                            </h3>
                            <p className="text-white/60 text-xs font-semibold mt-0.5">
                              {newUserForm.username ? `@${newUserForm.username}` : 'Create a staff account'}
                            </p>
                          </div>
                        </div>
                        <button
                          onClick={() => setShowAddUserModal(false)}
                          className="w-9 h-9 flex items-center justify-center bg-white/10 hover:bg-white/20 text-white/70 hover:text-white rounded-xl transition-all active:scale-90"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ...
                    {/* Form body — overlaps the banner */}
                    <div className="-mt-6 mx-4 bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
                      <form
                        onSubmit={async (e) => {
                          e.preventDefault();
                          if (!checks.length) { showNotification('Password must be at least 8 characters', 'error'); return; }
                          if (!checks.upper) { showNotification('Password must contain at least one uppercase letter', 'error'); return; }
                          if (!checks.lower) { showNotification('Password must contain at least one lowercase letter', 'error'); return; }
                          if (!checks.number) { showNotification('Password must contain at least one number', 'error'); return; }
                          if (!checks.special) { showNotification('Password must contain at least one special character (e.g. @, #, !)', 'error'); return; }
                          try {
                            ...
                            if (result?.success) {
                              ...
                              const msg = result?.message || result?.data?.error || result?.error || 'Failed to create user';
                      ...
                      >
                        ...
                        <div className="space-y-1.5">
                          <label className="flex items-center gap-1.5 text-[11px] font-black text-slate-500 uppercase tracking-widest">
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-bhutan-maroon/20 focus:border-bhutan-maroon/40 focus:bg-white transition-all font-semibold text-slate-800 outline-none placeholder:text-slate-400 text-sm"
                              placeholder="e.g. Tshering Dorji"
                              required
                            />
                            {newUserForm.fullName.trim().length > 1 && (
                              <Check className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-500" />
                            )}
                          </div>
                        </div>

                        {/* Username + Email in 2-col grid */}
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) => setNewUserForm({ ...newUserForm, username: e.target.value.replace(/\s/g, '_') })}
                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-bhutan-maroon/20 focus:border-bhutan-maroon/40 focus:bg-white transition-all font-mono font-bold text-slate-800 outline-none placeholder:text-slate-400 placeholder:font-sans placeholder:font-normal text-sm"
                                placeholder="john_doe"
                                required
                              />
                              {newUserForm.username.length >= 3 && /^[a-zA-Z0-9_]+$/.test(newUserForm.username) && (
                                <Check className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-500" />
                              )}
                            </div>
                            <p className="text-[10px] text-slate-400">Used to log in</p>
                          </div>
                          <div className="space-y-1.5">
                            <label className="flex items-center gap-1.5 text-[11px] font-black text-slate-500 uppercase tracking-widest">
                              <Mail className="w-3 h-3" /> Email <span className="normal-case font-normal text-slate-400">(opt.)</span>
                            </label>
                            <input
                              type="email"
                              value={newUserForm.email}
                              onChange={(e) => setNewUserForm({ ...newUserForm, email: e.target.value })}
                              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-bhutan-maroon/20 focus:border-bhutan-maroon/40 focus:bg-white transition-all font-semibold text-slate-800 outline-none placeholder:text-slate-400 placeholder:font-normal text-sm"
                              placeholder="staff@co.com"
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            <input
                              type={showAddUserModal ? 'password' : 'text'}
                              id="new-staff-password"
                              value={newUserForm.password}
                              onChange={(e) => setNewUserForm({ ...newUserForm, password: e.target.value })}
                              className="w-full px-4 py-3 pr-10 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-bhutan-maroon/20 focus:border-bhutan-maroon/40 focus:bg-white transition-all font-mono font-bold text-slate-800 outline-none placeholder:font-sans placeholder:font-normal placeholder:text-slate-400 text-sm"
                              placeholder="Min 8 chars · e.g. Staff@2024"
                              required
                              minLength={8}
                            />
                            <button
                              ...
                              onMouseDown={(e) => {
                                e.preventDefault();
                                const inp = document.getElementById('new-staff-password') as HTMLInputElement;
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                                  {([
                                    ...
                                    <span key={label} className={`text-[10px] font-bold flex items-center gap-0.5 transition-colors ${
                                      ok ? 'text-emerald-500' : 'text-slate-400'
                                    }`}>
                                      {ok ? <Check className="w-2.5 h-2.5" /> : <span className="w-2.5 h-2.5 inline-flex items-center justify-center text-slate-300">·</span>}
                                      {label}
                                    </span>
                                  ))}
                                </div>
                                <span className={`text-[10px] font-black ${strengthText}`}>{strengthLabel}</span>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex gap-3 pt-2">
                          <button
                            type="button"
                            onClick={() => setShowAddUserModal(false)}
                            className="flex-1 py-3 border-2 border-slate-200 rounded-xl hover:bg-slate-50 font-bold text-slate-500 text-sm transition-all active:scale-95"
                          >
                            Cancel
                          </button>
                          <button
                            type="submit"
                            disabled={score < 5}
                            className="flex-[2] py-3 bg-bhutan-maroon text-white rounded-xl font-black text-sm uppercase tracking-wider hover:bg-bhutan-maroon-dark shadow-lg shadow-bhutan-maroon/25 transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none flex items-center justify-center gap-2"
                          >
                    ...
                    <div className="px-6 pb-5 pt-3 text-center">
                      <p className="text-[10px] text-slate-400">Role is fixed to <span className="font-black text-slate-500">Staff</span>. Only 1 Admin account is allowed.</p>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="bg-white border border-slate-100 rounded-[28px] p-8 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-400 mt-1">v{appVersion}</p>
                </div>
                <button
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 mt-4">
                  ...
                  {currentUser?.role === 'admin' ? (
                    ...
                    <p className="text-xs font-bold text-amber-600 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
                      Only admin users can install updates. Please contact your administrator.
                    </p>
                  )}
                </div>
              )}
      ...
      <div className="flex flex-col xl:flex-row gap-6 xl:gap-10">
        ...
        <div className="w-full xl:w-72 shrink-0 space-y-2">
          ...
          <div className="p-8 bg-slate-900 rounded-[40px] border border-slate-800 relative overflow-hidden group">
            ...
            <p className="text-white font-bold text-xs">V {appVersion} - GST Ready</p>
