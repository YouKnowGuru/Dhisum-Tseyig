 ...
 *
 * BUG FIX C-4: also mixes in a per-install 256-bit secret (`installSecret`).
 * Without that file present, an attacker who can read the OS machine-id and
 * the build-time PEPPER still cannot reproduce the key. Without the install
 * secret the derivation throws to make accidental fallback impossible.
 */
function deriveKey(salt: Buffer): Buffer {
  ...
  try {
    ...
    // Combined with the install-secret check below, this only works once
    // before the key changes — i.e. is unusable for persisted data.
  ...
  // BUG FIX C-4: include install secret when available. If install secret is
  // missing, throw instead of silently allowing a week key derivation.
  const installSecretHex = (() => {
    const s = getInstallSecret();
    return s ? s.toString('hex') : null;
  })();
  if (!installSecretHex) {
    throw new Error('Encryption key derivation requires the install secret. Ensure initInstallSecret() has been called during app start-up.');
  }
